...
import { CheckCircle2, XCircle, Edit, MapPin, Phone, Mail, Globe, Loader2, Trash2 } from "lucide-react";
...
export default function AdminPendingTab({ listings, onListingUpdated, onEdit }) {
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      await Promise.all(selectedIds.map((id) =>
        base44.entities.CommunityListing.update(id, { status: "approved" })
      ));
      toast({ title: "Approved", description: `${selectedIds.length} listing${selectedIds.length !== 1 ? "s" : ""} approved and now live.` });
      setSelectedIds([]);
      onListingUpdated();
    } catch (err) {
      toast({ title: "Error", description: err.message, variant: "destructive" });
  ...
  const handleBulkDelete = async () => {
    if (!selectedIds.length) return;
    if (!confirm(`Delete ${selectedIds.length} listing${selectedIds.length !== 1 ? "s" : ""}? This cannot be undone.`)) return;
    setBulkLoading(true);
    try {
      await Promise.all(selectedIds.map((id) =>
        base44.entities.CommunityListing.delete(id)
      ));
      toast({ title: "Deleted", description: `${selectedIds.length} listing${selectedIds.length !== 1 ? "s" : ""} deleted.` });
      setSelectedIds([]);
      onListingUpdated();
    } catch (err) {
      toast({ title: "Error", description: err.message, variant: "destructive" });
  ...
  return (
    <div className="space-y-4">
      {/* Header row with select-all + bulk actions */}
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        {selectedIds.length === 0 && (
          <p className="text-sm text-muted-foreground">{pending.length} listing{pending.length !== 1 ? "s" : ""} awaiting review</p>
        )}
      </div>
      ...
      {pending.map((l) => (
        <div
          key={l.id}
          className={`bg-card border rounded-xl p-5 flex flex-col sm:flex-row gap-4 transition-colors cursor-pointer ${selectedIds.includes(l.id) ? "border-primary/60 bg-primary/5" : "hover:border-muted-foreground/30"}`}
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs text-muted-foreground">
              ...
              {l.website && <a href={l.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-primary" onClick={(e) => e.stopPropagation()}><Globe className="w-3 h-3" />{l.website.replace(/^https?:\/\//, "")}</a>}
            </div>
          ...
          {/* Per-row actions */}
          <div className="flex sm:flex-col gap-2 shrink-0 justify-end" onClick={(e) => e.stopPropagation()}>
            <Button
              size="sm"
              className="gap-1.5"
              style={{ background: "#097275" }}
              onClick={() => handleApprove(l)}
              disabled={!!loadingId || bulkLoading}
            >
